...
// 5. Add the "prunable" marker for the non-explicitly installed pkgs.
// 6. if --prune config set to true, then run npm prune.
...
function filterNodeVersion (data) {
  ...
  if (!supported.length) log.warn(data._id, "Not supported on node@"+process.version)
