...
function build (args, cb) {
  readAll(args, function (er, args) {
    ...
    chain
      ...
      // must rebuild bundles synchronously, because it messes with configs.
      , [chain].concat(args.map(function (a) { return function (cb) {
          rebuildBundle(a, cb)
        }}))
...
function rebuildBundle (pkg, cb) {
  ...
  fs.stat(bundle, function (er, s) {
    ...
    npm.commands.rebuild([], function (er, data) {
      conf.shift()
      log.warn([conf.list[0], "done"], "bundle rebuild")
      cb(er, data)
    })
  })
}
...
var autoActivate = function (pkg, cb) {
  ...
  log.warn([auto, conf.list[0], pkg._id], "auto-activate")
  if (!auto) {
